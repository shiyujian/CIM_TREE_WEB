...
import {actions} from '../store/itemPro';
import {connect} from 'react-redux';
import {bindActionCreators} from 'redux';
import {SubTree,Info,ToggleModal} from '../components/ItemPro';

@connect(
	state => {
		const {platform, project: {itemPro = {}} = {}} = state;
		return {platform, ...itemPro};
	},
	dispatch => ({
...
export default class ItemPro extends Component {
	...
	render() {
		...
		return (
			<div>
				<DynamicTitle title="分项工程" {...this.props}/>
				<Sidebar>
					<SubTree {...this.props}/>
				</Sidebar>
				<Content>

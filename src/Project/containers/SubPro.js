...
import {actions} from '../store/subPro';
import {connect} from 'react-redux';
import {bindActionCreators} from 'redux';
import {SubTree,Info,ToggleModal} from '../components/SubPro';

@connect(
	state => {
		const {platform, project: {subPro = {}} = {}} = state;
		return {platform, ...subPro};
	},
	dispatch => ({
		actions: bindActionCreators({...platformActions, ...actions}, dispatch),
	}),
)
export default class SubPro extends Component {
	...
	render() {
		...
		return (
			<div>
				<DynamicTitle title="分部工程" {...this.props}/>
				<Sidebar>
					<SubTree {...this.props}/>
				</Sidebar>
				<Content>

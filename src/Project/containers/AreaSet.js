...
import {actions} from '../store/areaSet';
import {connect} from 'react-redux';
import {bindActionCreators} from 'redux';
import {SubTree,Info,ToggleModal} from '../components/AreaSet';

@connect(
	state => {
		const {platform, project: {areaSet = {}} = {}} = state;
		return {platform, ...areaSet};
	},
	dispatch => ({
		actions: bindActionCreators({...platformActions, ...actions}, dispatch),
	}),
)
export default class AreaSet extends Component {
	...
	render() {
		...
		return (
			<div>
				<DynamicTitle title="区域设置" {...this.props}/>
				<Sidebar>
					<SubTree {...this.props}/>
				</Sidebar>
				<Content>

...
export default class SafetySystem extends Component {
    ...
    render () {
        ...
        return (
            <div>
                ...
                <Content>
                    {tableposible === 0 ? null : <Table {...this.props} />}
                </Content>
                <Additon {...this.props} />
    ...
    componentDidMount () {
        ...
        getworkTree({ code: Engicode }, { depth: 2 }).then(
            ({ children }) => {}
        );
    }
    ...
    onSelect (value = []) {
        ...
        for (let j = 0; j < worktree.length; j++) {
            if (worktree[j].code === valuecode) {
                ...
                const {
                    actions: { getdirtree, savecode, savepk, retn },
                    paren = {}
                } = this.props;
                retn(1);
                ...
                savepk(paren);
                getdirtree({ code: code.split('--')[1] }, { depth: 4 }).then(
                    ({ children, pk }) => {
                        ...
                        children.map((rst, index) => {
                            ...
                            if (newshuzu[index].length !== 0) {
                                rst.children.map((rst1, index1) => {
                                    ...
                                    if (
                                        newshuzu[index].children[index1]
                                            .length !== 0
                                    ) {
                                        rst1.children.map((rst2, index2) => {
                                            newshuzu[index].children[
                                                index1
                                            ].children[index2].on = 'three';
                                        });
                                    }
                                });
                            }

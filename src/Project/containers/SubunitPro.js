...
import {actions} from '../store/subunitPro';
import {connect} from 'react-redux';
import {bindActionCreators} from 'redux';
import {SubTree,Info,ToggleModal} from '../components/SubunitPro';

@connect(
	state => {
		const {platform, project: {subunitPro = {}} = {}} = state;
		return {platform, ...subunitPro};
	},
	dispatch => ({
		actions: bindActionCreators({...platformActions, ...actions}, dispatch),
	}),
)
export default class SubunitPro extends Component {
	...
	render() {
		...
		return (
			<div>
				<DynamicTitle title="子单位工程" {...this.props}/>
				<Sidebar>
					<SubTree {...this.props}/>
				</Sidebar>
				<Content>

...
export default class LevelEdit extends Component {
    ...
    name (filter, event) {
        ...
        leveldocs['C(发生事故可能造成的后果)'] =
            levelfile['C(发生事故可能造成的后果)'];
        leveldocs['D(总计)'] = levelfile['D(总计)'];
        leveldocs['E(人员暴露于危险环境中的频繁程度)'] =
            levelfile['E(人员暴露于危险环境中的频繁程度)'];
    ...
    save () {
        const {
            actions: { putdocument, getwxtype, levelEditVisible },
            wxtype = [],
            levelfile = {},
            typecode = {}
        } = this.props;

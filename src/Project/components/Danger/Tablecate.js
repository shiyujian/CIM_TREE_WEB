...
export default class Tablecate extends Component {
    ...
    rowclick = (record, index) => {
        const {
            actions: { getwxtype, savetypecode, setrst }
        } = this.props;
        savetypecode(record.code);
        getwxtype({ code: record.code }).then(rst => {
            if (rst === 'Meta List Not Found') {
                let newtypelists = [];
                const {
                    actions: { newtypelist }
                } = this.props;
    ...
    delet (file) {
        let dexx = file.on - 1;
        const {
            actions: { deletedocument, getWxlist }
        } = this.props;

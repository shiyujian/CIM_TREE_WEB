...
export default class dangerAddition extends Component {
                ...
                <Modal title="新增"
                    ...
                    <Form>
                        <FormItem label="安全隐患名称">
                            <Input value={docs.name} onInput={this.name.bind(this, filter)} />
                        </FormItem>
                        <FormItem label="安全隐患编码">
                            <Input value={docs.code} disabled={true} />
                        </FormItem>
                        <FormItem label="发生事故可能造成的后果">
                            <Input value={docs.things} onChange={this.things.bind(this, filter)} />
                        </FormItem>
                        <FormItem label="总计">
                            <Input value={docs.tatal} onChange={this.tatal.bind(this, filter)} />
                        </FormItem>
                        <FormItem label="人员暴露于危险环境中的频繁程度">
                            <Input value={docs.people} onChange={this.people.bind(this, filter)} />
                        </FormItem>
                        <FormItem label="事故发生的可能性">
                            <Input value={docs.posible} onChange={this.posible.bind(this, filter)} />
                        </FormItem>
                        <FormItem label="风险控制措施">
                            <Input value={docs.methode} onChange={this.methode.bind(this, filter)} />
                        </FormItem>
                        <FormItem label="风险级别">
                            <Select defaultValue="一级" onChange={this.levell.bind(this, filter)}>
    ...
    name(filter, event) {
        ...
        const { newtypelist = [], docs = {}, typecode = {}, actions: { setnewcode, setdocs } } = this.props;
    ...
    save() {
        const { actions: { patchdocument, levelAdding, getwxtype, postdocument, setdocs, setrst }, docs = {}, typecode = {}, newtypelist, setrsts } = this.props;
        patchdocument({ code: WXlevel }, {
            metalist: [
                {
                    "code": docs.code,
                    "name": docs.name,

...
export default class SumPlanCheck extends Component {
  ...
  async componentDidMount() {
    ...
    let topDir = await getScheduleDir({
      code: "the_only_main_code_SumPlanCheck"
    });
    if (!topDir.obj_type) {
      let postData = {
        name: "数据报送的顶级节点",
  ...
  async passon() {
    ...
    let code = "datareport_safetydoc_111201";
      ...
      related_objects: [
        {
          ...
          rel_type: "safetydoc_wp_zy" // 自定义，要确保唯一性
        }
      ],
    ...
    dataSource.map(item => {
      i++;
      docData.push({
        code: "abcdefg" + moment().format("YYYYMMDDHHmmss") + i,
        name: "abcdefg" + moment().format("YYYYMMDDHHmmss") + i,
        obj_type: "C_DOC",
        status: "A",

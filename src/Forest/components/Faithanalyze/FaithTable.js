...
export default class FaithTable extends Component {
	...
	search(index) {
        if(index === 1) {
            ...
            const {nurseryname} = this.state;
            // const suffix = nurseryname ? <Icon type="close-circle" onClick={this.emitEmpty} /> : null;
            return <div>
                <span>供苗商：</span>
                <Input className='forestcalcw3 mxw200' onPressEnter={this.nurserynamechange.bind(this)}/>
            </div>
        }
    }
    ...
    qury(index,param) {
        const {actions: {getHonestyNewSort, getHonestyNewTreetype},leftkeycode} = this.props;
        if(index === 1) {
            this.setState({loading1:true})
            getHonestyNewSort({},param)
            .then(rst => {
                ...
                try {
                    ...
                    rankData.forEach((data) => {
                        integrity.push(data['integrity'])
                        nurseryname.push(data['nurseryname'])
                    })  
                    let options1 = {
                        legend: {
                            data:['供苗诚信度']

...
export default class DeliverPlanTable extends Component {
    constructor(props) {
        super(props);
        this.state = {
            scheduleMaster: null,
    ...
    setConfig() {
        console.log('this.props', this.props.platform.tabs);
        gantt.addMarker({
    ...
    componentDidMount() {
        const { actions: { getProjectTree, getTotalPlanTime } } = this.props;
        //获取最初始的树节点
        let tasks = [];
        let parent = 1;
        let ganttData = [];
                ...
                getTotalPlanTime(rootPk).then(value => {
                            ...
                            if (project.start_time && project.end_time) {
                                ...
                                if (project.children && project.children.length > 0) {
                                    for (var s = 0; s < project.children.length; s++) {
                                        console.log('project.children', project.children[s]);
                    ...
                    for (let i = 0; i < tasks.length; i++) {
                        if (tasks[i].obj_type === 'project') {
                            ...
                            console.log('daration', duration);
                            ganttData.push({
                                id: i + 1,
                                text: tasks[i].name,
                                time_limit: duration.toString(),
                                start_time: moment(tasks[i].start_time).format('YYYY年MM月DD日'),
                                end_time: moment(tasks[i].end_time).format('YYYY年MM月DD日'),
                                details: tasks[i].pk,
                                start_date: moment(tasks[i].start_time).format('DD-MM-YYYY'),
    ...
    render() {
        //todo 总体进度
        this.setConfig();
        // const { projects = [], currentNode, unitChildren = [], childrenWorkPackages = [] } = this.props;
        return (
            <div>
                <div id="gannt_deliver_plan" style={{ width: '100%', height: '723px', float: 'left' }} />
            </div>
        );
    }
}

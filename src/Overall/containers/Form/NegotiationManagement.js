...
import { actions } from '../../store/Form/negotiationManagement';
import { TableListInitiate, TableListCommission, TableListFinished } from '../../components/Form/NegotiationManagement/';
...
@connect(
    state => {
        const {
            platform,
            overall: { negotiationManagement = {} }
        } = state;
        return { platform, ...negotiationManagement };
    },
    dispatch => ({
...
class NegotiationManagement extends Component {
    constructor (props) {
        super(props);
        this.state = {
    ...
    getAuditor = async () => {
        ...
        if (ConstructionData.code === 200 && SupervisorData.code === 200 && OwnerData.code === 200) {
            // 获取监理，施工，业主文书列表
            let ConstructionList = [], SupervisorList = [], OwnerList = [];
    ...
    render () {
        ...
        return (
            <Body>
                <Main>
                    <DynamicTitle title='洽商管理' {...this.props} />
                    <Sidebar>
                        {
...
export default NegotiationManagement;

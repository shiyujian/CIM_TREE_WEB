...
class TipsTable extends Component {
    ...
    clickTips (record, type) {
        ...
        if (type === 'DELETE') {
            ...
            patchData({ pk: record.id }, newData).then(rst => {
                if (rst.id) {
    ...
    query () {
        ...
        this.props.form.validateFields(async (err, values) => {
            ...
            if (values && values.worktime) {
                conditions.begin = moment(values.worktime[0]).format(
                    'YYYY-MM-DD'
                );
                conditions.end = moment(values.worktime[1]).format(
                    'YYYY-MM-DD'
                );
            }
                    ...
                    <Tabs
                        ...
                        tabBarExtraContent={
                            <div style={{ marginBottom: '10px' }}>
                                <Button
                                    type='primary'
                                    onClick={this.publishTipsClick.bind(this)}
                                >
                                    通知发布
                                </Button>
                                {toggleData.visible &&
    ...
    columns = [
        ...
        },
        {
            ...
            render: (text, record) => {
                if (record.update_time) {
                    return moment(record.update_time)
                        .utc()
                        .format('YYYY-MM-DD HH:mm:ss');
                } else {
        ...
        },
        {
            ...
            render: record => {
                return (
                    <span>
                        <a onClick={this.clickTips.bind(this, record, 'VIEW')}>
                            查看
                        </a>
                        &nbsp;&nbsp;|&nbsp;&nbsp;
                        <a onClick={this.clickTips.bind(this, record, 'EDIT')}>
                            修改
                        </a>
                        &nbsp;&nbsp;|&nbsp;&nbsp;
                        <Popconfirm
                            title='确定删除吗?'
                            onConfirm={this.clickTips.bind(
                                this,
                                record,
                                'DELETE'
                            )}

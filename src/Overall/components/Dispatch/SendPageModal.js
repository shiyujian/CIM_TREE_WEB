...
class SendPageModal extends Component {
    constructor (props) {
        super(props);
        this.state = {
            fileList: [],
            ...
            if (item && item.OrgCode) {
                ...
                if (item.children && item.children.length > 0) {
                    return (
                        <TreeNode
                            ...
                            value={`${item.ID}`}
                        >
                            {SendPageModal.loop(item.children, companyStatus)}
                        </TreeNode>
                    );
                } else {
                    return (
                ...
                if (item && item.Orgs && item.Orgs.length > 0) {
                    return (
                        ...
                        >
                            {SendPageModal.loop(item.Orgs, companyStatus)}
                        </TreeNode>
                    );
                } else {
                    return (
    ...
    sendSuccessCloseModal () {
        this.props.sendSuccessCloseModal();
    }
        ...
        validateFields((err, values) => {
            if (!err) {
                ...
                sentUsers.map(item => {
                    if (item) {
                        if (receivingUnit) {
                            receivingUnit = receivingUnit + ',' + item;
                        } else {
                            receivingUnit = item;
                        }
                    }
                });
                console.log('接收单位', receivingUnit);
                let copyUnit = '';
                copyUsers.map(item => {
                    if (item) {
                        if (copyUnit) {
                            copyUnit = copyUnit + ',' + item;
                        } else {
                            copyUnit = item;
                        }
                    }
                });
                console.log('抄送单位', copyUnit);
                let sendData = [{
                                    ...
                                    <Col span={12}>
                                        ...
                                        >
                                            {getFieldDecorator('receiveOrg', {
                                                ...
                                                >
                                                    {
                                                        SendPageModal.loop(orgTreeArrList)
                                                    }
                                                </TreeSelect>
                                            )}
                                        </FormItem>
...
export default Form.create()(SendPageModal);

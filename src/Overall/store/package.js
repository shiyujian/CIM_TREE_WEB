...
// 检查新的CODE是否已经存在
export const checkCodeIsActive = createFetchAction(
    `${SERVICE_API}/workpackages/code/{{code}}/`,
    []
);

// 获取单位工程或者子单位工程的施工包数据
export const getPackagesOK = createAction(
    '获取单位工程或者子单位工程的施工包数据'
);
export const getPackages = createFetchAction(
    `${SERVICE_API}/project-tree/code/{{code}}/?root=false&depth=3`,
    [getPackagesOK]
);

// 创建施工包流程
export const postInstance = createFetchAction(
    `${WORKFLOW_API}/instance/`,
    [],
    'POST'
);
// 指派执行人
export const putInstanceUser = createFetchAction(
    `${WORKFLOW_API}/instance/{{ppk}}/state/{{pk}}/`,
    [],
    'PUT'
);
// 启动流程
export const putInstanceStart = createFetchAction(
...
export const getInstance = createFetchAction(
...
// 提交要下一步审批
export const postLogEvent = createFetchAction(
...
// 流程抄送
export const postCarbonCopy = createFetchAction(
    `${WORKFLOW_API}/instance/{{pk}}/carbon-copy/`,
    [],
    'POST'
);
// 获取模板下载地址
export const getTemplates = createFetchAction(
    `${SERVICE_API}/excel/templates/`,
    []
);

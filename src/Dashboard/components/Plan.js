...
export default class Plan extends Component {
    ...
    options = [
        { label: '巡检路线', value: 'geojsonFeature_people', IconName: 'universal-access', },
    ...
    getPanelData(featureName) {
        ...
        switch (featureName) {
            ...
            case 'geojsonFeature_360':
                ...
                break;
            // case 'geojsonFeature_maproute':
            //     content = this.state.maproute;
            //     break;
        }
        return content;
    }
    ...
    onPeopleCheck(keys, checkItems) {
        ...
        this.setState({ userCheckedKeys: keys }, () => {
            ...
            getMapList().then((orgs) => {
                let arr1 = []
               
                let orgArr = orgs.map((or,index) => {
                    console.log("index",index)
                    if (or.RouteID === this.state.userCheckedKeys[0]) {
                        console.log("111", 111)
                        arr1.push(or)
                    }
                });
                console.log('arr1', arr1)
                me.setState({ mapList: arr1 });
                                                        ...
                                                        <ul>
                                                            {
                                                                this.state.mapList.map(item => {
                                                                    ...
                                                                    if(item.GPSTime){
                                                                        return <li>起点：{item.X}终点:{item.Y}</li>
                                                                    }

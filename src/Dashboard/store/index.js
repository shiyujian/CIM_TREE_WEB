...
export const getArea = createFetchAction(
    `${SERVICE_API}/loc-tree/code/LOC_ROOT/`,
    [getAreaOK]
);
...
export const getVedio = createFetchAction(
    `${SERVICE_API}/loc-tree/code/CAM_ROOT/`,
    []
);
export const getSafeMonitor = createFetchAction(
    `${SERVICE_API}/monitors/code/`,
    []
);
export const getUsers = createFetchAction(`${USER_API}/users/`, []);
export const getUsersOnline = createFetchAction(
    `${base}/main/api/users-online/?interval=30&minlat={{minlat}}&maxlat={{maxlat}}&minlng={{minlng}}&maxlng={{maxlng}}`,
    []
);
export const getUserOrgInfo = createFetchAction(
    `${SERVICE_API}/persons/code/{{CODE}}/?all=true`,
    []
);
export const getOrgs = createFetchAction(`${SERVICE_API}/org-tree/`, []);
export const getOrgsByCode = createFetchAction(
    `${SERVICE_API}/org-tree/code/{{CODE}}/`,
    []
);
export const getRiskProcess = createFetchAction(
...
export const getRiskProcessDetail = createFetchAction(
...
export const getCameraTree = createFetchAction(
...
export const getWks = createFetchAction(
    `${WORKFLOW_API}/instance/?code={{code}}`,
    []
);
const getDesignStage = createFetchAction(
    `${SERVICE_API}/metalist/designstage/`,
    []
);
// 批量获取设计成果
const getDocumentList = createFetchAction(
    `${SERVICE_API}/documentgetlist/`,
    []
);
// 批量模型下载地址
const getImodelInfoAc = createFetchAction(
    `${SERVICE_API}/documents/code/{{pk}}/?all=true`,
    []
);
// 获取安全规则制度
...
const getWorkpackages = createFetchAction(
...
// ----质量管理的模块代码
export const getChildrenOrgTree = createFetchAction(
    `${SERVICE_API}/project-tree/code/{{code}}/?root=false&depth=4`,
    []
);
export const fetchDefectDataByLoc = createFetchAction(
    `${base}/main/api/quality-defect/?project_location={{keyword}}`,
    []
);
// ----质量管理的模块代码
// 获取360全景图地址
const getVideo360List = createFetchAction(
    `${SERVICE_API}/metalist/video360list/`,
    []
);
...
// 获取轨迹列表
const getMapList = createFetchAction(
    `${FOREST_API}/tree/patrolpositions?routeid={{routeID}}`,
    []
);
// 存储二维展示点击后弹框数据
...
// 获取头部统计
export const getSamplingstat = createFetchAction(
    `${FOREST_API}/tree/samplingstat`,
    []
);
// 获取种植流程

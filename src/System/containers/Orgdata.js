...
@connect(
    state => {
        const {
            platform,
            system: { orgdata, persondata }
        } = state;
        return { platform, ...orgdata, ...persondata };
    },
    dispatch => ({
        actions: bindActionCreators(
            { ...platformActions, ...actions, ...action2 },
            dispatch
        )
    })
)
export default class Orgdata extends Component {
    ...
    setDataDel (data, participants, description) {
        // return;
        const {
            actions: { createWorkflow, logWorkflowEvent }
        } = this.props;
        let creator = {
            id: getUser().id,
            ...
            <div>
                ...
                <Content>
                    ...
                    {visibleCJ && (
                        <ToggleModalCJ2
                            {...this.props}
                            setDataCJ={this.setDataCJ.bind(this)}
                        />
                    )}
                    {visibleDel && (
                        <ToggleModalDel
                            {...this.props}
                            setDataDel={this.setDataDel.bind(this)}
                        />
                    )}
                    {visibleUpdate && (
                        <ToggleModalUpdate2
                            {...this.props}
                            setDataUpdate={this.setDataUpdate.bind(this)}
                        />
                    )}
                </Content>
            </div>

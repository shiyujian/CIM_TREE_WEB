...
import {
    SERVICE_API,
    DataReportTemplate_ConstructionUnits
} from '_platform/api';
import { Promise } from 'es6-promise';
import './TableOrg.less';
const TreeNode = TreeSelect.TreeNode;
export default class ToggleModalCJ2 extends Component {
    constructor (props) {
        super(props);
        this.state = {
            dataSource: [],
        ...
        return (
            ...
            >
                ...
                <div style={{ marginTop: '30px' }}>
                    <p>
                        <span>注：</span>1、请不要随意修改模板的列头、工作薄名称（sheet1）、列验证等内容。如某列数据有下拉列表，请按数据格式填写1；
                    </p>
    ...
    onok () {
        ...
        if (arr.length) {
            ...
            this.props.setDataCJ(this.state.dataSource);
        ...
        return data.map(item => {
            if (item.children && item.children.length > 0) {
                return (
                    ...
                    >
                        {ToggleModalCJ2.lmyloop(item.children, depth++)}
                    </TreeNode>
                );
            } else {
                return (
                    ...
                    if (this.state.repeatCode.indexOf(record.code) !== -1) {
                        return (
                            <Input
                                value={record.code}
                                style={{ color: 'red' }}
                                onChange={this.changeCode.bind(this, record)}
                            />
                        );
                    } else {
                        return (
                    ...
                    if (record.color === 'red') {
                        return (
                            <Input
                                value={record.type}
                                style={{ color: 'red' }}
                                onChange={this.changeType.bind(this, record)}
                            />
                        );
                    } else {
                        return (

...
import {
    SERVICE_API,
    FILE_API,
    DataReportTemplate_PersonInformation
} from '_platform/api';
...
export default class ToggleModal extends Component {
        ...
        const columns = [
                        ...
                        if (record.org) {
                            return (
                                <Input
                                    style={{ width: '60px' }}
                                    value={record.depart || ''}
                                    onChange={this.tableDataChange.bind(
                                        this,
                                        index
                                    )}
            ...
            },
            {
                ...
                render: (text, record, index) => {
                    if (record.editing === true) {
                        ...
                        return (
                            <span style={{ color: record.tel_red }}>
                                {record.tel}
                            </span>
                        );
                    }
            ...
            },
            {
                ...
                render: (text, record, index) => {
                    if (record.editing === true) {
                        ...
                        return (
                            <span style={{ color: record.email_red }}>
                                {record.email}
                            </span>
                        );
                    }
                ...
                render: (text, record, index) => {
                    if (record.editing === true) {
                        return (
                            <Select
                                placeholder='标段'
                                value={addition.sections || record.sections}
                                onChange={e => {
                                    console.log('e:', e);
                                    record.sections = e;
                                    this.forceUpdate();
                    ...
                    if (record.editing === true) {
                        return (
                            <Select
                                placeholder='苗圃'
                                showSearch
                                value={addition.tags || record.tags}
                ...
                render: (text, record, index) => {
                    if (record.editing === true) {
                        return (
                            <Select
                                placeholder='请选择角色'
                                value={addition.groups || record.groups}
                                onChange={this.changeRoles.bind(this, record)}
                                mode='multiple'
                                style={{ width: '100%' }}
                            >
                                <OptGroup label='苗圃角色'>
            ...
            },
            {
                ...
                render: record => {
                    return (
                        <Upload
                            beforeUpload={this.beforeUploadPic.bind(
                                this,
                                record
                            )}
                            accept={accept}
            ...
            >
                ...
                >
                    ...
                    <div style={{ marginTop: 20 }}>
                        注:&emsp;1、请不要随意修改模板的列头、工作薄名称（sheet1）、列验证等内容。如某列数据有下拉列表，请按数据格式填写；
                        <br />
                        &emsp;&emsp; 2、数值用半角阿拉伯数字，如：1.2
                        <br />
                        &emsp;&emsp; 3、日期必须带年月日，如2017年1月1日
                        <br />
                        &emsp;&emsp;
    ...
    beforeUploadPic (record, file) {
        ...
        fetch(`${FILE_API}/api/user/files/`, myInit).then(async resp => {
            ...
            loadedFile.download_url = this.covertURLRelative(
                loadedFile.download_url
            );
    ...
    async onok () {
        ...
        let pks = [];
        for (let i = 0; i < this.state.dataSource.length; i++) {
            let rst = await getOrgName({
                code: this.state.dataSource[i].depart
            });
            pks.push(rst.pk);
        }
    ...
    async handleExcelData (data) {
        const {
            actions: { getOrgReverse, getOrgName }
        } = this.props;
        data.splice(0, 1);
        let data_person,
            orgname = [],
        ...
        data_person = data.map((item, index) => {
            ...
            const sections =
                section.trim() == '' ? [] : section.trim().split(',');
            return {
                index: index + 1,
                name: item[1] || '',
                org: org || '',
    ...
    componentDidMount () {
        const {
            actions: { getAllUsers, getOrgList }
        } = this.props;

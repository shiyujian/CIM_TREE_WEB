...
export default (ID, service = '') => {
    ...
    const getRootDir = createFetchAction(`${SERVICE_API}/dir-tree/`, [
        getRootDirOK
    ]);
    const getDirOK = createAction(`${ID}_GET_DIR_OK_${suffix}`);
    const getDir = createFetchAction(
        `${SERVICE_API}/dir-tree/code/{{code}}/?depth=7`,
        [getDirOK]
    );
    const setCurrentCategory = createAction(
        `${ID}_SET_CURRENT_CATEGORY_${suffix}`
    );

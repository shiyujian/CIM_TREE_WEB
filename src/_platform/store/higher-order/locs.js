...
export default (ID, service = '') => {
    ...
    const getLocTree = createFetchAction(`${SERVICE_API}/loc-tree/`, [
        getLocTreeOK
    ]);
    const getLoc = createFetchAction(
        `${SERVICE_API}/locations/code/{{code}}/?all=true`
    );
    const postLoc = createFetchAction(`${SERVICE_API}/locations/`, 'POST');
    const putLoc = createFetchAction(
        `${SERVICE_API}/locations/code/{{code}}/?this=true`,
        'PUT'
    );
    const deleteLoc = createFetchAction(
        `${SERVICE_API}/locations/code/{{code}}/?this=true`,
        'DELETE'
    );

...
class Manager extends Component {
    ...
    componentWillReceiveProps(props){
        ...
        const {actions:{getSafetyManageTrain}} = this.props;
        const tempcode = code === "" ? `?project_code=${pcode}` : `?project_unit_code=${code}`
        getSafetyManageTrain({code:tempcode}).then(rst => {
    ...
    delete(index){
        const {actions:{delSafetyManageTrain}} = this.props
        let datas = this.state.dataSource;
        delSafetyManageTrain({id:datas[index].id}).then(rst => {
    ...
    setAddData(){
        const {actions:{getSafetyManageTrain,addSafetyManageTrain,getWkByCode}} = this.props;
        ...
        this.props.form.validateFields(async(err,values) => {
            if(!err){
                ...
                values.train_date = moment(values.train_date._d).format('YYYY-MM-DD');
                let putData = {
                    project:{
                        pk:project.pk,
                        code:project.code,
                ...
                addSafetyManageTrain({},putData).then(rst => {
                    try{
    ...
    onSearch(value){   
        ...
        getSafetyManageTrain({code:param}).then(rst => {
            let dataSource = this.handleData(rst)
        ...
        return (
            <div>
                ...
                <Modal 
					...
					>
					<AddManager props={this.props} state={this.state} />
				</Modal>
                <Modal 
					title="管理人员安全教育培训登记详情"
					width={760}
                    key={Math.random()*4}
					maskClosable={false}
					...
					>
					<ManagerDetail {...this.state.record} />
				</Modal> 
                <Preview />
                {/* <Modal 
					title="编辑安全目标"

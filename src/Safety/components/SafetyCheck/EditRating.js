...
class EditRating extends Component {
	...
	componentDidMount(){
		...
		this.setState({dataSource:record.reduce_item})
	}
	//table input 输入
	...
	onok(){
		this.props.form.validateFields(["check_item","item_property","score","ratio"],(err,values) => {
            if(!err){
				this.props.editCheckContent(this.state.dataSource,values)
            }
        }); 
	}
	//自定义校验规则：数字
...
export default Form.create({
    mapPropsToFields(props){
        return {
            check_item: {value: props.record.check_item},
            item_property: {value: props.record.item_property},
            score: {value: props.record.score},
            ratio: {value: props.record.ratio},
        }
    }
})(EditRating)
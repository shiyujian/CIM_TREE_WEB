...
import './Register.css';
import {EditTreatment}from '../components/Treatment';
...
class Treatment extends Component {
    ...
    onViewClick(record,index){
        ...
        let filed = {};
        filed.misc = record.survey_report.misc;
        filed.a_file = `${SOURCE_API}` + (record.survey_report.a_file).replace(/^http(s)?:\/\/[\w\-\.:]+/, '');
        filed.download_url = `${STATIC_DOWNLOAD_API}` + (record.survey_report.download_url).replace(/^http(s)?:\/\/[\w\-\.:]+/, '');
        filed.name = record.survey_report.name;
        filed.mime_type = record.survey_report.mime_type;
        openPreview(filed);
    }
    ...
    onDownClick(record,index){
        let apiGet = `${STATIC_DOWNLOAD_API}` + (record.survey_report.download_url).replace(/^http(s)?:\/\/[\w\-\.:]+/, '');
        ...
        getAccident({projectCode:project.code,unitCode:unitProject.code}).then(rst =>{
            ...
            if(rst.length!==0){
                for(let i=0;i<rst.length;i++){
                    ...
                    if(rst[i].survey_report!==null && rst[i].survey_report.length!==0){
                        data.survey_report = rst[i].survey_report[0];
                    }
        ...
        this.props.form.validateFields((err,values) => {
            if(!err){
                ...
                if(values.attachment.length!==0){
                    ...
                    array.push(postFile);
                    postData.survey_report = array;
                }
                if(postData.survey_report){  //if the postObj is not null
    ...
    render() {
        ...
        const columns = [{
            ...
            title: '调查报告',
            dataIndex: 'accDealReport',
            key:'accDealReport',
            render: (text,record,index) => {
                if(record.survey_report){
                   return <div>
        ...
        return (
            <div>
                ...
                >
                    <EditTreatment props={this.props} state={this.state}/>
                    </Modal>
                </Content>
                <Preview />
            </div>
...
export default Form.create()(Treatment);

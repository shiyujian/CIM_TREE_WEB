...
import {SERVICE_API, SCHEDULE_TOTAL_PLAN_URL,SCHEDULE_DGN_DOWNLOAD_URL,WORKFLOW_CODE,STATIC_DOWNLOAD_API,modelDownloadAddress } from '_platform/api';
...
export default class TotalApprovalForm extends Component {
                        ...
                        if(workflowID){
                            ...
                            getWorkflowById({ id: workflowID.workflowactivity.id }).then(instance => {
                                ...
                                if(instance && instance.subject){
                                    ...
                                    this.uploadModelFile = JSON.parse(subject.uploadModelFile);
                                    this.fdbConnectStr =  JSON.parse(subject.fdbConnectStr);
                    ...
                    <div >
                        ...
                        <Card>
                            ...
                            <Row style={{marginTop:10}}>
                                ...
                                <Col span={8}>
                                    <a style={{textAlign:'center',display: 'block'}} onClick={this.modelView.bind(this)}>预览模型</a>
                                </Col>
                            </Row>
                        </Card>
                        <Card className='mb10'>
        ...
        putFlow(data,postdata).then( rst=>{
            ...
            if(rst && rst.creator){
                ...
                getDocument(documentCode).then((documents)=>{
                    if(documents && documents.code && documents.extra_params){
                        //获取文档中的模型数据
                        let changeUploadModelFile = [];
                        let FDBStr = '';
                        if(this.fdbConnectStr){
                            FDBStr = this.fdbConnectStr;
                        }
                        if(documents.extra_params.uploadModelFile){
                            changeUploadModelFile = documents.extra_params.uploadModelFile;
                        }
                        if(this.uploadModelFile){
                            changeUploadModelFile.push(this.uploadModelFile)
                        }
    ...
    modelView(){
        ...
        if(this.file){
            // if(this.uploadModelFile && this.uploadMD5File){
            if(this.uploadModelFile){
                let fileName = this.uploadModelFile.name.substring(0,this.uploadModelFile.name.indexOf('.zip'));
                try{

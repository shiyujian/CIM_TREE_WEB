...
class Login extends Component {
	...
	render() {
		...
		const loginTitle = require('./images/logo02.png');
		...
		return (
			<div className="login-wrap">
				...
				{/*综合管理平台登录方案*/}
				{/*<iframe src="http://xaweb.simulate.com:8080/eccms/ssoIndex.html" style={{display: 'none'}}></iframe>*/}
			</div>
		);
	}
	...
	loginFunc(data, loginType, values) {
		...
		login({}, data).then(rst => {
			if (rst && rst.id) {
				//综合管理系统用户登录,合同管理和档案管理模块
				this.loginAnthorFun(rst.username);
	...
	//综合管理系统用户登录,合同管理和档案管理模块
	loginAnthorFun(username){
		//username="test102601"  //根据参数改变
	
		// 登录
		$.ajax({
			url : window.config.ZHLoginServletCMD,
			//'http://xaweb.simulate.com:8080/eccms/servlet/LoginServlet.cmd',
			dataType : 'jsonp',
			type : 'post',
			data : {
				action : 'tologin',
			...
			success : function(response, textStatus, jqXHR) {
			   ...
			   //alert("登录成功！！！！")
				...
				if (response.login != "true") {
					...
					//Fix.Runtime.setError(msg);
					return;
				}
			

				window.localStorage.login_saveInfo = username;
				window.localStorage.firstLogin = "";
				window.localStorage.Local = response.local;

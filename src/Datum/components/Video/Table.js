...
import { Table, Spin, message,Modal,Row, Col, Input, Form, Icon, Button, } from 'antd';
...
export default class GeneralTable extends Component {
	...
	componentDidUpdate(prevProps,prevState){
		...
		if(searchvideovisible && (searchvideo!=prevProps.searchvideo || Doc!=prevProps.Doc) && Doc.length>0){
			this.filter()
		}
	}
	...
	filter(){
		...
		let arr = Doc.filter(doc => 
			(searchvideo.searchName? doc.name.indexOf(searchvideo.searchName) != -1: true) &&
			(searchvideo.searchType? doc.extra_params.type.indexOf(searchvideo.searchType) != -1: true) &&
			(searchvideo.searchDate_begin? moment(doc.extra_params.time).isAfter(searchvideo.searchDate_begin) : true) &&
			(searchvideo.searchDate_end? moment(doc.extra_params.time).isBefore(searchvideo.searchDate_end): true)
		)
			...
			<div>
				...
				<div>
					...
					<Row style={{marginTop:10}}>
						<Button  onClick={this.onCancel.bind(this) } style={{float:'right'}}type="primary">关闭</Button>
					</Row>
				</div>

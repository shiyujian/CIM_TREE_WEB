...
import { Modal, Table, Button, Input, Upload, Icon, Form, message, Divider } from 'antd';
...
export default class Lmap extends Component {
    ...
    initMap () {
        ...
        if (CUS_TILEMAP) {
            L.tileLayer(
                `${CUS_TILEMAP}/Layers/_alllayers/LE{z}/R{y}/C{x}.png`
            ).addTo(this.map);
        }
    ...
    createMarker = (cameraData, oldLayerGroup) => {
        if (oldLayerGroup.length) oldLayerGroup.map(layer => layer.remove());
        if (cameraData) {
    ...
    handleDeleteInspectRecord = record => {
        ...
        delInspectRecordDocument({ pk: record.key }).then(resp => {
            getProjectDetail({ pk: record.engineeringPk }).then(res => {
                ...
                const index = recordFile.indexOf(
                    recordFile.find(item => item.pk === record.key)
                );
                extra.recordFile.splice(index, 1);
                bindCameraToProject(
                    { pk: record.engineeringPk },
                    postData
                ).then(rst => {
    ...
    handleDownloadInspectRecord = record => {
        document
            ...
            .insertAdjacentHTML(
                'afterend',
                '<iframe src="' +
                    `${DOWNLOAD_FILE}${record.file.download_url}` +
            ...
            },
            {
                ...
                render: (text, record, index) => (
                    <span>
                        <a
                            onClick={() => {
                                Modal.confirm({
                                    title: '删除巡检记录',
                                    content: `确定删除 ${
                                        record.inspectContent
                                    } ？`,
                                    okText: '确认',
                                    cancelText: '取消',
                                    onOk: () =>
                                        this.handleDeleteInspectRecord(record)
                                });
                            }}
                        >
                            删除
                        ...
                        <a
                            onClick={() =>
                                this.handleBrowseInspectRecord(record)
                            }
                        >
                            浏览
                        </a>
                        ...
                        <a
                            onClick={() =>
                                this.handleDownloadInspectRecord(record)
                            }
                        >
                            下载
                        </a>
            ...
            <div className='map-container' style={{ position: 'relative' }}>
                <div ref='appendBody' className='l-map r-main'>
                    ...
                    >
                        <Camera
                            style={{
                                display: this.state.showCamera ? '' : 'none'
                            }}
                            {...this.state.video}
                            ref='camera'
                    ...
                    {this.state.uploadModalvisible && (
                        ...
                        >
                            <UploadModal onOk={this.handleUploadRecord} />
                        </Modal>
                    )}
...
class UploadModal extends Component {
    ...
    beforeUploadPicFile = file => {
        ...
        // 上传图片到静态服务器
        const {
            actions: { uploadStaticFile, deleteStaticFile }
        } = this.props;
        ...
        uploadStaticFile({}, formdata).then(resp => {
            ...
            this.props.form.setFieldsValue({
                attachment: resp.id ? attachment : null
            });
        });
        return false;
    };
        ...
        return (
            <div>
                <Form>
                    ...
                    <FormItem {...formItemLayout} label='功能图标' hasFeedback>
                        {getFieldDecorator('attachment', {
                            ...
                            <Upload
                                listType='text'
                                beforeUpload={this.beforeUploadPicFile}
                                onRemove={this.removePicFile}
                            >
                                <Button>

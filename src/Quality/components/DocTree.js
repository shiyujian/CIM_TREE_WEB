...
export default class DocTree extends Component {
    ...
    render () {
        ...
        return (
            <div>
                <Select
                    placeholder='图纸阶段'
                    style={{ width: 212, margin: 12 }}
                    value={stage}
                    onChange={this.toggleStage.bind(this)}
                >
    ...
    onSelect ([code]) {
        const {
            actions: { changeSidebarField, getDocuments }
        } = this.props;
    ...
    componentDidMount () {
        const {
            actions: { getBlueprintstageMeta, changeSidebarField, getDir }
        } = this.props;

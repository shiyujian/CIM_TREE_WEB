...
import {actions as actions4}from '../store/subitem'
...
import {USER_API, SERVICE_API, WORKFLOW_API,JYPMOD_API,UPLOADFILE_API,SERVICE_USER_ID,SERVICE_USER_PWD,previewWord_API,STATIC_DOWNLOAD_API,Danwei_WordTemplate} from '_platform/api';
...
@connect(
    ...
    dispatch => ({
        actions: bindActionCreators({ ...previewActions,...actions, ...platformActions,...actions3,...actions4}, dispatch),
        cellActions: bindActionCreators({ ...actions2 }, dispatch),
    }),
)
export default class DwJLShow extends Component {
    constructor(props){
        super(props);
        this.state = {
            danweiName:'',
    ...
    componentDidMount(){
      //  console.log(this.props); 
        ...
        getWorkflow({pk:nowDanwei.extra_params.wfid}).then(rst=>{
            ...
            let txts = JSON.parse(nowDanwei.extra_params.jlcomment);
    ...
    show(){
        let {exchangeWordFile}  = this.props.actions;
        let postData = 
        {
            doc_template_url:Danwei_WordTemplate
        };
        postData.jianli = this.state.jianliName;
        for(let i = 0;i<this.state.values.length;i++){
            postData[`input${i+1}`] = this.state.values[i]||'';
        }
        for(let i = 0;i<this.state.selects.length;i++){
            postData[`select${i+1}`] = this.state.selects[i]||'';
        }
        for(let i = 0;i<this.state.texts.length;i++){
            postData[`txt${i+1}`] = this.state.texts[i]||'';
        }
        exchangeWordFile({}, postData).then(exfile_res  => {
            const modalContent = JSON.parse(exfile_res);
            console.log('modalContent',modalContent);
            let file = {...modalContent};
            this.setState({modalContent:modalContent})
            file.a_file = previewWord_API + file.a_file;
            let { openPreview } = this.props.actions;
            openPreview(modalContent);
        })
    }
    download() {
        let downliad_iframe =  document.createElement('iframe');
        downliad_iframe.src = this.state.modalContent.download_url;
        downliad_iframe.style.display = 'none';
        document.getElementById('root').appendChild(downliad_iframe)
        // this.state.modalContent && document.querySelector('#root').insertAdjacentHTML('afterend',
        //     '<iframe src="' + `${this.state.modalContent.download_url}` + '" style="display: none"></iframe>')
    }
    render() {
            ...
            <div
             style = {{backgroundColor:'#fff',position:'absolute',zIndex:'90',top:'70px',width:'100%'}}>
                <div>
                        <Button onClick = {()=>{
                            window.history.back();
                        }} type='primary' style={{margin:'10px',position:'relative',left:'50px',top:'20px'}}>返回</Button>
                </div>
                ...
                <Card>
                    ...
                    <Row style={{marginBottom:'10px'}}>
                        ...
                        <Col span={9}>
                        <div style = {{width:'95%',height:'100px'}}>
                            共<Input disabled  className = 'dwInput' style = {{width:'40px'}}/>项,经查符合要求<Input disabled className = 'dwInput' style = {{width:'40px'}}/>项，
                ...
                <div>
                    <div style={{ width: '300px', margin: '0 auto' }}>
                        ...
                        <Button onClick = {this.show.bind(this)} type='primary' style={{ margin: '15px' }}>预览表单</Button>
                        <Button disabled = {!this.state.modalContent} onClick = {this.download.bind(this)} type='primary' >表单下载</Button>
                    </div>
                </div>
            </div>
        );

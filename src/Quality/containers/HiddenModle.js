...
export default class HiddenModle extends Component {
    ...
    createMarker () {
        ...
        let marker = L.marker(geo.geometry.coordinates, {
            icon: dangerIcon,
            title: geo.riskContent
        });
        console.log('marker', marker);
        marker.bindPopup(
            L.popup({ maxWidth: 240 }).setContent(this.genPopUpContent(geo))
        );
    ...
    render () {
        ...
        return (
            ...
            >
                <Row>
                    <h2 style={{ textAlign: 'center' }}>安全隐患详情</h2>
                </Row>
                <Row style={{ marginTop: 15 }}>
                    <Col span={8}>
                        <span style={{ fontSize: 16 }}>{`隐患内容：${
                            detail.problemType
                        }`}</span>
                    </Col>
                    <Col span={6} style={{ float: 'right' }}>
                        <span style={{ fontSize: 16 }}>{`发现人员：${
                            detail.fsPeople
                        }`}</span>
                    </Col>
                </Row>
                <Row>
                    <Col span={16}>
                        <span style={{ fontSize: 16 }}>{`单位工程：${
                            detail.unitName
                        }`}</span>
                    </Col>
                    <Col span={6} style={{ float: 'right' }}>
                        <span style={{ fontSize: 16 }}>{`监理工程师：${
                            detail.jlPeople
                        }`}</span>
                    </Col>
                </Row>
                <Row>
                    <Col span={8}>
                        <span style={{ fontSize: 16 }}>{`整改措施：${
                            detail.zgcuoshi
                        }`}</span>
                    </Col>
                    <Col span={6} style={{ float: 'right' }}>
                        <span style={{ fontSize: 16 }}>{`整改负责人：${
                            detail.resPeople
                        }`}</span>
                    </Col>
                </Row>

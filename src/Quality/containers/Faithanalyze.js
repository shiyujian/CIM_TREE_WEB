...
export default class Faithanalyze extends Component {
    ...
    setTreeTypeOption (rst) {
        if (rst instanceof Array) {
            let treetypeoption = rst.map(item => {
                return (
                    <Option key={item.TreeTypeNo} value={item.TreeTypeName}>
                        {item.TreeTypeName}
                    </Option>
                );
            });
    ...
    onSelect (value = []) {
        let keycode = value[0] || '';
        const {
            actions: { getTree, gettreetype }
        } = this.props;
        this.setState({ leftkeycode: keycode });
        // 标段
        getTree({}, { parent: keycode }).then(rst => {
    ...
    onExpand (expandedKeys, info) {
        ...
        getTree({}, { parent: keycode, paginate: false }).then(rst => {
            if (rst instanceof Array) {

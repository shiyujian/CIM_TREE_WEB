...
export default class Practicalfill extends React.Component {
	...
	render() {
		...
		let columns = [{
			...
			title: '实际开始时间',
			dataIndex: 'basic_params.tech_params.actual_process.start_time',
			...
			title: '实际结束时间',
			dataIndex: 'basic_params.tech_params.actual_process.end_time',
			render: (text, record, index) => {
				return (
					<EditableCell
						value={text}
			...
			title: '单位',
			dataIndex: 'extra_params.task.actual.unit',
			render: (text, record, index) => {
				return (
					<EditableCell
						value={text}
			...
			title: '数量',
			dataIndex: 'extra_params.task.actual.count',
			render: (text, record, index) => {
				return (
					<EditableCell
						value={text}
			...
			title: '实际完成百分比',
			dataIndex: 'extra_params.task.actual.percent',
			render: (text, record, index) => {
				return (
					<EditableCell
						value={text}
			...
			title: '实际完成量',
			dataIndex: 'extra_params.task.actual.amount',
			render: (text, record, index) => {
				return (
					<EditableCell
						value={text}
			...
			title: '备注',
			dataIndex: 'extra_params.task.actual.remark',
			render: (text, record, index) => {
				return (
					<EditableCell
						value={text}
			...
			<Form>
				<Row gutter={24}>
					<Col span={24}>
						<Row gutter={24}>
							<Col span={6}>
								<FormItem {...Practicalfill.layout}
								          label="项目">
									<Cascader placeholder="请选择建设项目"
									          allowClear={false}
									          displayRender={Practicalfill.displayRender}
	...
	submit() {
		...
		if (editIdx.length === 0) {
			...
			editIdx.map((idx) => {
				...
				for (let key in work) {
					if (key === 'start_time') {
						editData.basic_params.actual_process.start_time = work['start_time']
					}
					if (key === 'end_time') {
						editData.basic_params.actual_process.end_time = work['end_time']
					}
					if (key === 'unit') {
						editData.extra_params.task.actual.unit = work['unit']
					}
					if (key === 'count') {
						editData.extra_params.task.actual.count = work['count']
					}
					if (key === 'percent') {
						editData.extra_params.task.actual.percent = work['percent']
					}
					if (key === 'amount') {
						editData.extra_params.task.actual.amount = work['amount']
					}
					if (key === 'remark') {
						editData.extra_params.task.actual.remark = work['remark']
					}
				}

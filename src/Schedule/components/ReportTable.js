...
export default class ReportTable extends Component {
    ...
    constructor(props){
        ...
        this.state={
            ...
            connectStr: "", // city maker Server  connect
                        ...
                        <Card className='mb10'>
                            <Row gutter={10} >
                                <Col span={24}>
                                    <Row>
                                        <Col span={24}>
                                            <p style={{marginBottom:5}}>请上传施工进度模型:</p>
                                        </Col>
                                    </Row>
                                    <Row gutter={10} >
                                        <Col span={24} >
                                            {
                                                this.state.uploadModelFile?
                                                <p>{this.state.uploadModelFile.name}
                                                    <a href="javascript:;" style={{marginLeft:'5px'}}
                                                    onClick={()=>{
                                                    this.setState({uploadModelFile:null});
                                                    this.setProgress(false);
                                                }}>删除</a></p>:
                                                ''
                                            }
                                            {
                                                this.state.uploadFDBFile?
                                                <p>{this.state.uploadFDBFile.name}
                                                    <a href="javascript:;" style={{marginLeft:'5px'}}
                                                    onClick={()=>{
                                                    this.setState({uploadFDBFile:null});
                                                }}>删除</a></p>:
                                                ''
                                            }
                                            <Dragger  
                                            ...
                                            showUploadList={true}
                                            multiple={true}
                                            beforeUpload = {this.uploadModelFileChange.bind(this)}
                                            >
                                                <p className="ant-upload-drag-icon">
                                                    <Icon type="inbox" />
                                                </p>
                                                <p className="ant-upload-text">请点击或将文件拖入此区域内上传（文件为FDB格式）</p>
                                            </Dragger>
                                            {isShowProgress ? <Progress percent={progressPercent} strokeWidth={5} status={progressStatus} /> : ''}
                                        </Col>
                                    </Row>
                                </Col>
                            </Row>
    ...
    submitWorkflow(){
        ...
        let next_state=this.nextStates[0].to_state[0].id;
        let next_states=[];
        let participants=[];
        participants.push(this.supervisionMember)
        next_states.push({
            state: next_state,
            participants: participants,
            deadline: this.approvalTime,
            remark: null
        })
        ...
        let postdata = {
            ...
            state : state,
            executor : executor,
            action : this.nextStates[0].action_name,
            note: this.nextStates[0].action_name,
            attachment: null
        }
        let data={
        ...
        // start Progress
        this.setProgress(true, 80, 'active');

        // Post to FDBServer
        this.postFDBServer().then(connectStr => {
            ...
            if(this.memberCarbon.length>0){
                ...
                carbonCopy(data,postCarbonData).then( (value)=>{
                    if(value.ccmessage_set){
                        this.setProgress(true, 100, 'success');
                        notification.success({
                            message: '流程抄送成功',
                            duration: 2
                        });
                    }else{
                        this.setProgress(true, 100, 'exception');
                        notification.error({
                            message: '流程抄送失败',
                            duration: 2
                        })
    ...
    uploadModelFileChange(file){
        ...
        let len = type.length;
        if(type[len-1] === 'fdb' || type[len-1] === 'FDB'){
            this.setState({uploadModelFile:file});
        } else {
            notification.error({
                message: '请上传FDB文件',
                duration: 2
    ...
    postFDBServer() {
        ...
        return postScheduleModelFDB({},formdata,{
            Authorization: 'Basic ' + btoa(USER + ':' + PASSWORD)
        }).then(response => {
            // connectStr
            if(response && response.connectStr){
    ...
    postFDBServer() {
        ...
        const formdata = new FormData();
        formdata.append('a_file',uploadModelFile);
        formdata.append('name',uploadModelFile.name);
        formdata.append('scheduleTable',JSON.stringify(scheduleMaster));
    ...
    //Progress
    setProgress = (visible, percent = 0, status = '') => {
        this.setState({

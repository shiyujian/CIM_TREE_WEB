...
export default class Blueprint extends Component {
    ...
    edit (blueprint) {
        const {
            actions: { changeTableField, getDocument }
        } = this.props;
    ...
    remove (blueprint) {
        const {
            sidebar: { node },
            actions: { deleteDocument, getDocuments, toggleEditing }
        } = this.props;
                ...
                let nodes = [];
                {
                    files.map((file, index) => {
                        nodes.push(
                            <a
                                key={index}
                                onClick={this.previewFile.bind(this, file)}
                            >
                                文件预览
                            </a>
                        );
    ...
    previewFile (file) {
        const {
            actions: { openPreview, closeLoading }
        } = this.props;
        openPreview(file);
        setTimeout(() => {
            closeLoading();

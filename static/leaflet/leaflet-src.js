    ...
    L.TileLayer = L.Class.extend({
        ...
        getTileUrl: function (tilePoint) {
            ...
            if (this.options.tiletype && this.options.tiletype == "arcgis") {
                var x = tilePoint.x.toString(16);
                var y = tilePoint.y.toString(16);
                x = "C" + pad(x,8);
                y = "R" + pad(y,8);
                return lUrl + "/" + (lZoom - 1) + "/" + y + "/" + x + ".png";
            }
            if (this.options.storagetype == "1") {
                var prex = ".png";
                var aLURL = lUrl.split("/");
                if (aLURL[aLURL.length - 1] == "yx" && aLURL[aLURL.length - 1] != "yxsl") {
                    prex = ".jpg";
                }
                return lUrl + "/" + lZoom + "/" + tilePoint.y + "/" + tilePoint.x + prex;

            } else {
                if (this.options.multi.length > 0) {
                    for (var i = 0; i < this.options.multi.length; i++) {
                        if (lZoom <= this.options.multi[i].maxZoom && lZoom > this.options.multi[i].minZoom) {
                            lUrl = this.options.multi[i].url;
                            break;
                        }
                    }
                }
                var lastURL = L.Util.template(lUrl, L.extend({
                    s: this._getSubdomain(tilePoint),
                    z: lZoom,
                    x: tilePoint.x,
                    y: tilePoint.y
    ...
    */
    ...
    L.ImageOverlay = L.Class.extend({
        ...
        _initImage: function () {
            ...
            this._image.style.zIndex = 0;  //add by lck 20131101
    ...
    */
    ...
    L.Marker = L.Class.extend({
        ...
        initialize: function (latlng, options) {
            L.setOptions(this, options);
            this.happenedit = false;  //是否发生过编辑
            this._visible = true;  //初始化为显示  LCK
        ...
        setVisible: function (visible) {
            ...
            this._icon.parentNode.style.visibility = visible;
        },
    ...
    */
    ...
    L.LayerGroup = L.Class.extend({
        ...
        setVisible: function (visible) {
            this._visible = visible;
            for (var i in this._layers) {
                this._layers[i].setVisible(visible);
            }
        },

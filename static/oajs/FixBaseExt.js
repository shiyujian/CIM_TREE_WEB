...
function CreateWin(dialogCfg, urlParams) {
	...
	var url="";
	if (dialogCfg.resourceUrl.indexOf("http://")==-1)
		url="http://"+window.location.host+ Fix.App.appHost + dialogCfg.resourceUrl;  
	else
		url=dialogCfg.resourceUrl;
...
{	
	if (typeof action=="undefined")
	    action="cancel";  //add by chenmin 20151123
...
Fix.PagePrint=function(obj,portrait){    
  ...
  try {	
		...
		if (portrait==null)
		  factory.printing.portrait = true;  //设定为横印
		else
		   factory.printing.portrait = portrait;  //设定为横印   

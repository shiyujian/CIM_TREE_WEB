$(function(){
    var $target = $('#property'),flow,$slider=$('#slider');
    ...
    var toolBts = [{"start round":'起点'},{"task":'普通节点'},{"chat":"选择节点"},{"fork":"分流节点"},{"join":"合流节点"},{"end round":"终点"}];
...
//            headBtns:["new","open","save","undo","redo","reload","print"],//如果haveHead=true，则定义HEAD区的按钮
    ...
    function initFlow() {
        $.get(SERVER + temp_id +'/',{}).success(function (rep) {
           ...
           // flow.setNodeRemarks(remark);
            ...
            defs.appendChild(GooFlow.prototype.getSvgMarker("arrow_done",GooFlow.prototype.color.done));
            defs.appendChild(GooFlow.prototype.getSvgMarker("arrow_undo",GooFlow.prototype.color.undo));
            defs.appendChild(GooFlow.prototype.getSvgMarker("arrow_processing",GooFlow.prototype.color.processing));
    ...
    function setItemColor(itemDom, type, markType) {
        ...
        if(type=="node"){
            itemDom.css({"background-color": GooFlow.prototype.color[markType],"border-color": GooFlow.prototype.color[markType]});
        }else if(type=="line"){
           if(GooFlow.prototype.useSVG!=""){
               itemDom.childNodes[1].setAttribute("stroke", GooFlow.prototype.color[markType]);
               itemDom.childNodes[1].setAttribute("marker-end","url(#arrow_" + markType + ")");
